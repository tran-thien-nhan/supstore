...
@section('header')
    ...
    <!-- Các thẻ Meta OG -->
    <meta property="og:title" content="{{ $meta_brand_title }}" />
    <meta property="og:url" content="{{ URL::current() }}" />
    <meta property="og:type" content="website" />
    <!-- End Các thẻ Meta OG -->
    <title>{{ $meta_brand_title }}</title>
@endsection
@section('product_content')
    <!-- phần hiển thị -->
    <div class="d-flex">
        <div class="container row row-cols-1 row-cols-md-2 row-cols-sm-2 row-cols-lg-4">
            @foreach ($brand_by_id as $key => $brand_id)
                <div class="container col mb-4">
                    <div class="card border-0">
                        <a href="{{ URL::to('/chi-tiet-san-pham/' . $brand_id->product_id) }}"
                            class="d-flex justify-content-center align-items-center">
                            <img style="height: 16rem; width: 14rem; object-fit: cover;"
                                src="{{ asset('public/uploads/product/' . $brand_id->product_image) }}"
